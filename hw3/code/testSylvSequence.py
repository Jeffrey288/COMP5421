...
rects[0] = [101, 61, 155, 107] # we are doing subpixel interpolation

rects_q14 = np.zeros((num_frames, 4), dtype=float)
rects_q14[0] = [101, 61, 155, 107] # we are doing subpixel interpolation

# display the first frame
display_frame(frames[:, :, 0], rects[0], rects_q14[0])
plt.savefig(f'../writeup/q2,3_frame{0}')    
...
for frame_idx in range(1, num_frames):
    ...
    p = LucasKanadeBasis.LucasKanadeBasis(frames[:,:,frame_idx-1], frames[:,:,frame_idx], rect, bases)
    rects[frame_idx] = [rect[0] + p[0], rect[1] + p[1], rect[2] + p[0], rect[3] + p[1]]

    # baseline comparison
    rect_q14 = rects_q14[frame_idx-1]
    # when talking about p, we are referring to the *accumulative* deviation from the first frame
    p_new = p_prev + LucasKanade.LucasKanade(frames[:,:,frame_idx-1], frames[:,:,frame_idx], rect_q14) 
    ...
    if np.linalg.norm(p_star - p_new) < 2: # will lose track of car if this number is too small
        ...
        rects_q14[frame_idx] = [rect_q14[0] + p_step[0], rect_q14[1] + p_step[1], rect_q14[2] + p_step[0], rect_q14[3] + p_step[1]]
        p_prev = p_star
    else:
        rects_q14[frame_idx] = rects_q14[frame_idx-1]

    if frame_idx % 100 == 0:
        display_frame(frames[:, :, frame_idx], rects[frame_idx], rects_q14[frame_idx])
        plt.savefig(f'../writeup/q2,3_frame{frame_idx}')    
...
def animate(frame_idx):
    ...
    patch1 = ax.add_patch(get_patch(rects[frame_idx]))
    patch2 = ax.add_patch(get_patch(rects_q14[frame_idx], color="green"))
    return im, patch1, patch2
