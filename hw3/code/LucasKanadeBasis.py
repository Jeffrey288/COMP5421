...
def LucasKanadeBasis(It, It1, rect, bases):
	...
	# construct B and BB^T
	bases_vec = bases.reshape((-1, bases.shape[2])) # B
	bases_proj = bases_vec @ bases_vec.T # BB^T
	bases_1minus = np.eye(rectH * rectW) - bases_proj
	...
	for _ in range(20):
		...
		p_step = np.linalg.lstsq(bases_1minus @ A, bases_1minus @ b)[0]

		# Update our offset by p←p+Δp
		if np.linalg.norm(p_step) > 1e-5: # 0.7 works well, 0.1 makes no visible difference
			p += p_step	
		else:
