...
warped_im_check = warpA_check.warp(im_gray, A, output_shape)
warped_im = warpA.warp(im_gray, A, output_shape)

axes[1, 0].imshow(warped_im, cmap=plt.get_cmap('gray'))
axes[1, 0].set_title('warped')
# axes[1, 1].imshow(warped_im_check, cmap=plt.get_cmap('gray'))
# axes[1, 1].set_title('warped check')
# uncomment delete axis[1,1] to use
