...
for epoch in range(max_iter + 1):
    ...
    with torch.no_grad():
        for x, y in valid_dataset_loader:
            ...
            cum_valid_correct += torch.eq(y_pred, y).sum().detach().item()
            loss = loss_function(y_probs, y)
            cum_valid_loss += loss.detach().item() * valid_dataset_size # loss is tensor
    ...
    for i, (x, y) in enumerate(train_dataset_loader):
        ...
        correct = torch.eq(y_pred, y).sum().detach().item()
        cum_correct += correct
